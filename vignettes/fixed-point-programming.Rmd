...
I originally followed Mark Seemann's [blog](https://blog.ploeh.dk/2019/05/13/peano-catamorphism/), which in turn uses `Fix()` as in Bartosz Milewski's [article on F-Algebras](https://bartoszmilewski.com/2017/02/28/f-algebras/). Furthermore, a [Haskell Wikibook section](https://en.wikibooks.org/wiki/Haskell/Fix_and_recursion) explores `fix()` and recursion.

# Fixed points

Cheating a bit—`Fix()` just wraps another layer of recursion, it does not have a recursive type signature like the Haskell's `fix()`—I defined the `Fix()` and `unFix()` functions as follows.
